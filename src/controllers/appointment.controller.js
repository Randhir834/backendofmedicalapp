 ...
 //
 // Notes on time slots:
 // - Doctor timing is stored as HH:mm strings (24-hour) for two sessions
 // - Available slots are generated in 60-minute steps and formatted as "hh:mm AM/PM"
...
export async function createAppointment(req, res, next) {
  try {
    ...
    if (!Number.isFinite(fee) || fee < 0) {
      return res.status(400).json({ success: false, message: "Valid fee is required" });
    }

    const doctor = await Doctor.findById(doctorId)
      .select("fullName qualification clinicAddress experience timing approvalStatus isOnline")
      .lean();
...
export async function rescheduleMyAppointment(req, res, next) {
  try {
    ...
    const doctor = await Doctor.findById(appointment.doctorId).select("timing approvalStatus").lean();
    if (!doctor || doctor.approvalStatus !== "approved") {
      return res.status(404).json({ success: false, message: "Doctor not found" });
    }
